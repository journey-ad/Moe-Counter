...
fs.readdirSync(themePath).forEach(theme => {
  ...
  imgList.forEach(filename => {
    ...
    if (!imgExts.includes(path.extname(filename).toLowerCase())) return

    const imgPath = path.resolve(currentThemePath, filename)
    const char = path.parse(filename).name
...
function getCountImage(params) {
  let { name, count, theme = 'moebooru', padding = 7, prefix = -1, offset = 0, align = 'top', scale = 1, pixelated = '1', darkmode = 'auto', site, base64 = false } = params
  ...
  const defs = uniqueChar.reduce((ret, cur) => {
    let { width, height, data: imgData, filename } = themeList[theme][cur]
    width *= scale
    height *= scale

    // Use image url if base64 is false, and optimize the bandwidth usage.
    if (!base64) {
      imgData = `${site}/theme/${theme}/${filename}`
    }

    y = Math.max(y, height)

    ret = `${ret}
    <image id="${cur}" width="${toFixed(width, 5)}" height="${toFixed(height, 5)}" href="${imgData}" />`
  ...
  const parts = countArray.reduce((ret, cur) => {
    ...
    const image = `${ret}
    <use x="${toFixed(x, 5)}"${yOffset ? ` y="${toFixed(yOffset, 5)}"` : ''} href="#${cur}" />`
